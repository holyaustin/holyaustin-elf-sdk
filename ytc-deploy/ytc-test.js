...
async function main() {
    ...
    //Get amount:
    const amount = 10000000;//parseInt(prompt("Enter base tokens amount in decimals: "));
    ...
    const trancheIndex = 0;//parseInt(prompt(`Enter index of the expiry date of the tranche [${expiries}]: `))
    ...
    const n = 1;//parseInt(prompt("Enter number of compounds to do: "));
    ...
    console.log("User approves YTC contract to spend base token on its behalf - SUCCESSFUL");
    console.log(`Goerli block explorer link: https://goerli.etherscan.io/tx/${tx.hash}`);

    //2. Compound!
    console.log(`YTC - compound ${n} times with ${amount} ${baseTokenName}`);
    tx = await ytc.compound(n, trancheAddress, balancerPoolId, amount, 10, {
      gasLimit: 2500000
      });
    await tx.wait();
